...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(OpenLoop_SYSID_M, 0);
  ...
  OpenLoop_SYSID_initialize();
  cli();
  sei();

  /* External Mode initialization */
  errorCode = extmodeInit(OpenLoop_SYSID_M->extModeInfo,
    (extmodeSimulationTime_T *)rteiGetPtrTFinalTicks
    (OpenLoop_SYSID_M->extModeInfo));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(OpenLoop_SYSID_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(OpenLoop_SYSID_M);
