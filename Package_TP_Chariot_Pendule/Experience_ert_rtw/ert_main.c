...
int main(void)
{
  ...
  init();
  MW_Arduino_Init();
  rtmSetErrorStatus(Experience_M, 0);

  /* Set Final Simulation Time in Ticks */
  errorCode = extmodeSetFinalSimulationTime((extmodeSimulationTime_T) 6000);
  ...
  errorCode = extmodeInit(Experience_M->extModeInfo, (extmodeSimulationTime_T *)
    rteiGetPtrTFinalTicks(Experience_M->extModeInfo));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Experience_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(Experience_M);
